      ...
      <div class="form-container">
        ...
        <form action="/modifierProduit" method="POST">
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control" id="designation" name="designation" rows="3">{{ $produit->designation }}</textarea>
          </div>
          <div class="mb-3">
            <label for="image" class="form-label">Image</label>
            <input type="text" class="form-control" id="image" name="image" required value="{{ $produit->image }}">
          </div>
          <div class="mb-3">
          ...
          <div class="mb-3">
            ...
            <select class="form-select" id="etat_produit" name="etat_produit" required>
                @foreach($etat_produits as $etat_produit)
                    <option value="{{ $etat_produit }}" @if($produit->etat_produit == $etat_produit) selected @endif>{{ $etat_produit }}</option>
                @endforeach
            </select>
          </div>
          <div class="mb-3">
